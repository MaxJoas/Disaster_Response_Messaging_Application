...
def test_prediction_endpoint_returns_prediction(flask_test_client):
    ...
    # When
    response = flask_test_client.post("/go", json=json.loads(post_json))
    ...
    assert isinstance(response.get("predictions")[0], dict)
    assert len(response.get("predictions")[0]) == 36
    assert response.get("predictions")[0]["related"] == 1

    for i in categories:
        if i != "related":
            assert response.get("predictions")[0][i] == 0
